...
class PurchaseOrderController extends Controller
{
					...
					for($i=0; count($product_cat_ids)>$i; $i++){
						...
						if(!in_array($product_slug, $product_slugs)){
							...
							if($product_id!=''){
								...
								$barcode1			= isset($item_size_result[0]->product_barcode)?$item_size_result[0]->product_barcode:'';
								$barcode2	 		= isset($item_size_result[0]->barcode2)?$item_size_result[0]->barcode2:'';
								$barcode3	 		= isset($item_size_result[0]->barcode3)?$item_size_result[0]->barcode3:'';
	...
	public function deleteInwardStock(Request $request,$id){
		try {
			...
			$is_sell=isset($inward_stock_result->is_sell)?$inward_stock_result->is_sell:'';
			
			if($is_sell=='Y'){
				return redirect()->back()->with('error', 'You Can\'t delete this!');
			}
			...
			$purchase_date	= date('Y-m-d',strtotime($inward_stock_result->created_at));
			...
			if(count($inward_stock_products) > 0){
				...
				foreach($inward_stock_products as $product){
					...
					$daily_stock_product = DailyProductPurchaseHistory::whereBetween('created_at', [$purchase_date." 00:00:00", $purchase_date." 23:59:59"])->where('product_id',$product->product_id)->where('size_id',$product->size_id)->where('branch_id',$branch_id)->first();
					$stock_id=isset($daily_stock_product->id)?$daily_stock_product->id:'';
					if($stock_id!=''){
						$branch_stock_product_daily_price = DailyProductPurchaseHistory::where('id',$stock_id)->first();
						$branch_stock_product_daily_price->total_qty = $branch_stock_product_daily_price->total_qty - $product->product_qty;
						$branch_stock_product_daily_price->total_ml = $branch_stock_product_daily_price->total_ml - $product->total_ml;
						$branch_stock_product_daily_price->closing_stock = $branch_stock_product_daily_price->closing_stock - $product->product_qty;
						$branch_stock_product_daily_price->closing_stock_ml = $branch_stock_product_daily_price->closing_stock_ml - $product->total_ml;
						
						if($branch_stock_product_daily_price){
							
							$branch_stock_product_daily_price->updated_at = Carbon::now();
							$branch_stock_product_daily_price->save();
						}		
					}
					
					//echo '<pre>';print_r($branch_stock_product_daily_price);exit;
	...
	public function warehouseStockReport(Request $request){
		//$fileName = "warehouse-stock-report-" . date('Ymd') . ".xlsx";
		
		$fileName = "warehouse-stock-" . date('Y-m-d') . ".xls"; 
		...
		// Column names 
		$fields = array('ID', 'Product Name', 'Category', 'Subcategory', 'Size', 'Price', 'Warehouse Stock');
		...
		$stock_product = BranchStockProducts::with('stockProduct')->whereHas('stockProduct', function($q){
				$q->where('w_qty','!=','0')->orWhere('c_qty','!=','0');
			})->where('branch_id',$branch_id)->where('stock_type','counter');
		$stock_product=$stock_product->get();
		
		//$stock_product = BranchStockProducts::where('branch_id',$branch_id)->where('stock_type','counter')->get();
		if(count($stock_product)>0){
			$i=1;foreach($stock_product as $row){
				$selling_price	= $row->stockProduct->selling_price;
				$w_qty			= $row->stockProduct->w_qty;
				$lineData 		= array($i, $row->product->product_name, $row->product->category->name, $row->product->subcategory->name, $row->size->name, $selling_price, $w_qty);
...
public function filterData(&$str){ 
	...
	public function stockTranfer(Request $request){
		...
		try{
			...
			//echo '<pre>';print_r($stock_product[0]->stockProduct->selling_price);exit;
	...
	public function product_stock_upload(Request $request){
		...
		if($file){
			...
			foreach ($importData_arr as $importData) {
				...
				//echo 'product_barcode '.$product_barcode.' category-'.$category.' -type'.$type.' -brand_name'.$brand_name.' size'.$size.' opening_stock-'.$opening_stock;exit;
